...
import { applyOutcomeLogCommand, outcomeCommand } from "./commands/outcome.js";
...
export function createProgram(argv: string[] = process.argv): Command {
  ...
  program
    ...
    .option(
      "--yes",
      "Confirm overwriting existing files (required for --force in non-interactive/--json)",
    )
  ...
  program
    ...
    .option(
      "--reason <reason>",
      "Optional reason/note (free text, or one of: caused_bug|wasted_time|contradicted_requirements|wrong_context|outdated|other)",
    )
  ...
  program
    ...
    .option("-j, --json", "Output JSON")
    .addHelpText("after", () =>
      formatCommandExamples(["usage", "usage --json", "usage --json > usage.json"]),
    )
  ...
  program
    ...
    .option(
      "--no-interactive",
      "Never run the installer; print the exact update instructions instead",
    )
    .addHelpText("after", () =>
      formatCommandExamples([
        "update --check",
  ...
  program
    ...
    .option("-j, --json", "Output JSON")
    .addHelpText("after", () =>
      formatCommandExamples(["starters", "starters --json", "init --starter typescript"]),
    )
  ...
  program
    ...
    .action(
      async (status: string, rules: string, opts: any) =>
        await outcomeCommand({ ...opts, status, rules }),
    );

  // --- Outcome Apply ---
  program
    .command("outcome-apply")
  ...
  onboard
    ...
    .action(
      async (sessionPath: string, opts: any) =>
        await onboardCommand({ ...opts, read: sessionPath }),
    );
  ...
  onboard
    ...
    .action(
      async (sessionPath: string, opts: any) =>
        await onboardCommand({ ...opts, markDone: sessionPath }),
    );
