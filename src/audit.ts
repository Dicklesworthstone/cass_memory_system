import { z } from "zod";
import { type CassRunner, cassExport } from "./cass.js";
import { fillPrompt, type LLMIO, llmWithFallback, PROMPTS } from "./llm.js";
import { getActiveBullets } from "./playbook.js";
import type { AuditViolation, Config, Playbook } from "./types.js";
...
export async function scanSessionsForViolations(
  ...
  if (activeBullets.length > 100) {
    warn(
      `Audit running with ${activeBullets.length} rules. This may exceed context limits or degrade performance.`,
    );
  }
    ...
    await Promise.all(
      chunk.map(async (sessionPath) => {
        try {
          ...
          const result = await llmWithFallback(AuditOutputSchema, prompt, config, io);
