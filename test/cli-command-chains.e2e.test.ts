...
describe("CLI Command Chains E2E", () => {
  ...
  describe("Init → Context → Mark → Context Flow", () => {
    test.serial(
      "marking a bullet as helpful increases its score in subsequent context calls",
      () => {
        // Step 1: Initialize
        ...
        const contextResult1 = runCm(
          ["context", "validate user input security", "--json"],
          testDir,
        );
  ...
  describe("Playbook Add → Mark → Score Flow", () => {
    test.serial(
      "marking bullets changes their effective score and maturity",
      () => {
        // Initialize
  ...
  describe("Playbook Add → Forget → Undo Flow", () => {
    test.serial(
      "undo restores a forgotten bullet to active state",
      () => {
        // Initialize
        runCm(["init", "--json"], testDir);
        ...
        // Add a bullet
        const addResult = runCm(
          ["playbook", "add", "Test rule for undo workflow", "--category", "testing", "--json"],
          testDir,
        );
        expect(addResult.exitCode).toBe(0);
        const bulletId = (JSON.parse(addResult.stdout) as any).data.bullet.id;
        ...
        const forgetResult = runCm(
          ["forget", bulletId, "--reason", "Testing undo flow", "--json"],
          testDir,
        );
    ...
    test.serial(
      "undo --feedback removes the last feedback event",
      () => {
        // Initialize
        runCm(["init", "--json"], testDir);
        ...
        // Add a bullet
        const addResult = runCm(
          ["playbook", "add", "Test rule for feedback undo", "--category", "testing", "--json"],
          testDir,
        );
        expect(addResult.exitCode).toBe(0);
        const bulletId = (JSON.parse(addResult.stdout) as any).data.bullet.id;
    ...
    test.serial(
      "undo --hard requires explicit confirmation (--yes) in non-interactive mode",
      () => {
        runCm(["init", "--json"], testDir);
  ...
  describe("Stats → Top → Stale Flow", () => {
    test.serial(
      "stats, top, and stale reflect the same playbook state",
      () => {
        // Initialize
        runCm(["init", "--json"], testDir);
  ...
  describe("Why → Mark → Why Flow", () => {
    test.serial(
      "why command shows updated feedback after marking",
      () => {
        // Initialize
        runCm(["init", "--json"], testDir);
        ...
        // Add a bullet
        const addResult = runCm(
          ["playbook", "add", "Test rule for why flow", "--category", "testing", "--json"],
          testDir,
        );
        expect(addResult.exitCode).toBe(0);
        const bulletId = (JSON.parse(addResult.stdout) as any).data.bullet.id;
  ...
  describe("Doctor → Init --force → Doctor Flow", () => {
    test.serial(
      "doctor detects missing files and init --force resolves issues",
      () => {
        ...
        // Add some data
        runCm(
          ["playbook", "add", "Test rule for doctor flow", "--category", "testing", "--json"],
          testDir,
        );
  ...
  describe("Doctor Flags", () => {
    test.serial(
      "doctor --dry-run does not create or modify files",
      () => {
    ...
    test.serial(
      "doctor --fix --no-interactive applies safe fixes without hanging",
      () => {
    ...
    test.serial(
      "doctor --json --self-test includes selfTest results only when requested",
      () => {
    ...
    test.serial(
      "doctor --json --fix --dry-run includes a fix plan and makes no changes",
      () => {
  ...
  describe("Quickstart → Context Flow", () => {
    test.serial(
      "quickstart provides valid workflow that context can follow",
      () => {
        // Initialize
        runCm(["init", "--json"], testDir);
        ...
        const contextResult = runCm(["context", "implement a new feature", "--json"], testDir);
  ...
  describe("New User Onboarding Flow", () => {
    test.serial(
      "fresh install → init → context → mark helpful → doctor",
      () => {
        ...
        logger.step("patch-config", "info", "Patched config.cassPath", {
          cassPath: configJson.cassPath,
        });
        ...
        logger.info("Onboarding flow complete", {
          totalDurationMs:
            initDurationMs + addDurationMs + contextDurationMs + markDurationMs + doctorDurationMs,
        });
      },
      { timeout: 30000 },
    );
  ...
  describe("Error Recovery Flow", () => {
    test.serial(
      "corrupted config.json → doctor --fix resets config and restores clean runs",
      () => {
        ...
        expect(
          fixJson.data.fixResults.some((r: any) => r.id === "reset-config" && r.success === true),
        ).toBe(true);
